...
public class QuestLogScrollingList : MonoBehaviour
{
   ...
   private QuestLogButton InstantiateQuestLogButton(Quests quest, UnityAction selectAction)
   {
   ...
   private void UpdateScrolling(RectTransform buttonRectTransform)
   {
      ...
      if(buttonYMax > contentYMax){
         contentRectTransform.anchoredPosition = new Vector2(
            contentRectTransform.anchoredPosition.x, buttonYMax - scrollRectTransform.rect.height
         );
      } else if(buttonYMin < contentYMin) 
      {
         contentRectTransform.anchoredPosition = new Vector2(
            contentRectTransform.anchoredPosition.x, buttonYMin
         );
      }
   }
}
